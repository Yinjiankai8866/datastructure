...
public class MinSpanTreePrim {
    ...
    /**
     * 普里姆算法实现最小生成树：先初始化拿到第一个顶点相关联的权值元素放到数组中－》
     * 找到其中权值最小的顶点下标－》
     * 再根据该下标，将该下标顶点相关联的权值加入到数组中－》
     * 循环遍历处理
     */
    public void prim() {
        ...
        for (int i = 1; i < size; i++) {
            ...
            // 算法核心所在：
            // 将目标顶点到各个顶点的权值与当前tempWeight数组中的权值做比较，如果前者比后者到某个顶点的权值更小，
            // 将前者到这个顶点的权值更新入后者。
            tempWeight[minId] = 0;
            for (int j = 1; j < size; j++) {
    ...
    public static void main(String[] args) {
        MinSpanTreePrim graph = new MinSpanTreePrim();
        graph.createGraph(9);
        graph.prim();
    }
